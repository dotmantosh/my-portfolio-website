<template>
  <div class="main">
    <div @click="$emit('setComponentToMount', 'mainshow')" class="back--icon">
      <i class="fas fa-long-arrow-alt-left"></i>
    </div>

    <div class="form--title">
      <h1>Edit {{oneWorkInfo.workTitle}}</h1>
    </div>
    <form @submit.prevent class="form">
        ...
        <label for="work-ui" class="work-ui-label">
          ...
          <p class="text-center">
            ...
            <small
              v-if="workUi"
              class="is-uploading"
            >Wait a moment {{workUi.name}} is uploading ...</small>
          </p>
        </label>
      ...
      <div class="form-item">
        <label for="work-title">Title of work</label>
        <input
      ...
      <button
        ...
        type="buton"
        class="work-submit"
      >{{isClicked? 'Updating work...': 'Update'}}</button>
    </form>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState(["oneWorkInfo", "workInfo"])
  },
  mounted() {
    if (this.oneWorkInfo) {
      Object.keys(this.oneWorkInfo).map(
        key => (this.workInfoForm[key] = this.oneWorkInfo[key])
      );
      /* this.workInfoForm.workTitle = this.workInfo.workTitle;
      this.workInfoForm.workLink1 = this.workInfo.workLink1;
      this.workInfoForm.workLink2 = this.workInfo.workLink2;
      this.workInfoForm.workLink3 = this.workInfo.workLink3;
      this.workInfoForm.workLink4 = this.workInfo.workLink4;
      this.workInfoForm.workDescription = this.workInfo.workDescription;
      this.workInfoForm.workUi = this.workInfo.workUi; */
    }
  },
  methods: {
    ...mapActions(["setWorkInfo", "setOneWorkInfo"]),
    async onFileChange(e) {
      let files = e.target.files[0];
      this.workUi = this.$refs.file.files[0];
      if (!files) {
        return;
      }
      const formData = new FormData();
      formData.append("files", this.workUi);
      this.workUi = "";
      try {
        await WorkService.updateWorkUi(this.$route.params.id, formData);
        const newWorkInfo = (await WorkService.getWorks()).data;
        this.setWorkInfo(newWorkInfo);
        const oneWork = this.workInfo.filter(work => {
          return work._id === this.$route.params.id;
        });
        this.setOneWorkInfo(oneWork[0]);
        this.$emit("setComponentToMount", "mainshow");
    ...
    async updateWork() {
      try {
        this.isClicked = true;
        await WorkService.updateWork(this.workId, this.workInfoForm);
        const newWorkInfo = (await WorkService.getWorks()).data;
